...
export function ProductPageMinimal({
  ...
  if (isLoading || !product) {
    return (
      <div className={`min-h-screen bg-perce-bg ${className}`}>
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
          <div className='flex gap-2 mb-8'>
            <div className='h-4 w-12 bg-perce-surface animate-pulse' />
            <div className='h-4 w-4 bg-perce-surface animate-pulse' />
            <div className='h-4 w-12 bg-perce-surface animate-pulse' />
            <div className='h-4 w-4 bg-perce-surface animate-pulse' />
            <div className='h-4 w-32 bg-perce-surface animate-pulse' />
          </div>
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-12'>
            <div className='aspect-square bg-perce-surface animate-pulse' />
            <div className='space-y-6 pt-4'>
              <div className='h-8 w-3/4 bg-perce-surface animate-pulse' />
              <div className='h-6 w-1/4 bg-perce-surface animate-pulse' />
              <div className='h-4 w-full bg-perce-surface animate-pulse' />
              <div className='h-4 w-5/6 bg-perce-surface animate-pulse' />
              <div className='h-12 w-full bg-perce-surface animate-pulse mt-8' />
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`product-page-minimal bg-perce-bg ${className}`}>
      {/* ═══════════════════════════════════════════════
          BREADCRUMB
          ═══════════════════════════════════════════════ */}
      <div className='border-b border-perce-line py-4 px-4 sm:px-6 lg:px-8'>
        <div className='max-w-7xl mx-auto'>
          <nav className='text-sm text-perce-ink-muted font-light'>
            <a href='/' className='hover:text-perce-ink'>
              {t("product.home")}
            </a>
            <span className='mx-2 text-perce-ink-subtle'>&gt;</span>
            {product.categoryName && (
              <>
                <a href='/shop' className='hover:text-perce-ink'>
                  {product.categoryName}
                </a>
                <span className='mx-2 text-perce-ink-subtle'>&gt;</span>
              </>
            ...
            <span className='mx-2 text-perce-ink-subtle'>&gt;</span>
            <span className='text-perce-ink'>{product.name}</span>
          </nav>
        </div>
      </div>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12'>
          ...
          <div className='flex gap-3'>
            ...
            <div className='flex-1'>
              <div className='overflow-hidden bg-perce-surface relative'>
                {product.available && (
                  <span className='absolute top-3 start-3 z-10 bg-perce-cta text-perce-ink-inverse text-xs font-medium px-3 py-1'>
                    {t("new") || "New"}
                  </span>
                )}
                <img
          ...
          <div className='space-y-3'>
            ...
            <div className='flex items-start gap-2'>
              <h1 className='flex-1 text-xl lg:text-2xl font-medium text-perce-ink leading-tight'>
                {product.name}
              </h1>
              <div className='flex items-center gap-1.5 flex-shrink-0 pt-0.5'>
                <button
                  onClick={() => setShowShareMenu(!showShareMenu)}
                  className='w-7 h-7 flex items-center justify-center border border-perce-line hover:border-perce-ink transition-colors'
                  aria-label={t("product.share")}>
                  <Share2 className='w-3 h-3 text-perce-ink-secondary' />
                </button>
                {showWishlist && (
                  <button
                    type='button'
                    ...
                    className={cn(
                      ...
                      wishlisted
                        ? "bg-perce-surface text-perce-sale hover:bg-perce-line"
                        : "bg-perce-surface-raised text-perce-ink-secondary hover:bg-perce-surface",
                    )}
                    aria-label={
            ...
            {showShareMenu && (
              <div className='flex items-center gap-2 pb-1'>
                <a
                  ...
                  className='w-7 h-7 flex items-center justify-center border border-perce-line hover:border-perce-ink hover:bg-perce-cta-hover hover:text-perce-ink-inverse transition-colors'
            ...
            <div>
              {hasDiscount ? (
                <div className='flex items-baseline gap-2'>
                  <span className='text-lg font-semibold text-perce-ink'>
                    {formatMoney((product.discountPrice as number))}
                  </span>
                  <span className='text-sm text-perce-ink-muted line-through'>
              ...
              {/* <p className='text-[10px] text-perce-ink-muted mt-0.5'>
            ...
            {/* Stock status */}
            {product.available ? (
              <p className='text-sm text-perce-success font-bold flex items-center gap-1'>
                <Check className='w-4 h-4' />
                {t("in_stock")}
              </p>
            ) : (
              <p className='text-sm text-perce-sale font-bold'>
                {t("out_of_stock")}
              </p>
            )}
            ...
            <div className='pt-2 border-t border-perce-line space-y-3'>
              {/* Variant Selector */}
              ...
              <div className='flex items-stretch gap-2'>
                {product.available && (
                  <div className='flex items-center border border-perce-line-strong rounded-md shrink-0 overflow-hidden'>
                    <button
                      onClick={decrementQty}
                      disabled={quantity <= 1}
                      className='w-9 h-11 flex items-center justify-center hover:bg-perce-surface disabled:opacity-30 transition-colors'
                ...
                <Button
                  ...
                  size='lg'
                  variant='outline'
                  className='flex-1 border-perce-ink text-perce-ink hover:bg-perce-surface rounded-md h-11 py-0 text-sm font-medium shadow-none transition-all gap-2'
              ...
              {product.available && (
                <Button
                  size='lg'
                  className='w-full bg-perce-cta hover:bg-perce-cta-hover text-perce-ink-inverse rounded-md py-4 text-sm font-semibold shadow-none hover:shadow-lg transition-all'
            ...
            <Accordion type='single' collapsible defaultValue='about' className='pt-2 border-t border-perce-line'>
              {(() => {
                // Legacy `fragranceInfo` may still exist on records carried
                // over from the previous fragrance storefront. Percé does not
                  ...
                  <>
                    {hasAbout && (
                      <AccordionItem value='about'>
                        ...
                        <AccordionContent>
                          <div className='bg-perce-surface p-4 space-y-3'>
                            {product.description && (
                              <ColoredDescription
                                text={product.description}
                                className='text-sm text-perce-ink font-medium leading-relaxed whitespace-pre-line'
                              />
                            )}
                    ...
                    {shippingCopy && (
                      <AccordionItem value='shipping'>
                        ...
                        <AccordionContent>
                          <p className='text-sm text-perce-ink-secondary font-medium whitespace-pre-line'>
                            {shippingCopy}
                          </p>
                        </AccordionContent>
                      </AccordionItem>
                      ...
                      <AccordionContent>
                        <div className='space-y-3'>
                          {(pageContent?.faqs ?? []).map((faq, idx) => (
                            <div key={idx}>
                              <p className='text-sm font-bold text-perce-ink'>
                                {isAr ? faq.questionAr || faq.question : faq.question}
                              </p>
                              <p className='text-sm text-perce-ink-secondary font-medium mt-0.5 whitespace-pre-line'>
                        ...
                        <AccordionContent>
                          <div className='space-y-1'>
                            {product.specifications.map((spec, idx) => (
                              <div
                                key={idx}
                                className='flex justify-between items-baseline py-1 border-b border-perce-line last:border-0'>
                                <span className='text-sm font-semibold text-perce-ink-secondary'>{spec.label}</span>
                                <span className='text-sm text-perce-ink font-bold'>{spec.value}</span>
                              </div>
                            ))}
                          </div>
                        </AccordionContent>
      ...
      {product.available && (
        <div
          ...
          className={cn(
            "fixed inset-x-0 border-t border-perce-line bg-perce-surface-raised shadow-[0_-2px_10px_rgba(36,29,25,0.06)] transition-transform duration-300 lg:bottom-0",
          ...
          <div className='max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-2.5 flex items-center gap-2 sm:gap-3'>
            ...
            <div className='min-w-0 flex-1'>
              <p className='hidden sm:block text-xs font-medium text-perce-ink truncate'>
                {product.name}
              </p>
              <div className='flex items-center gap-2'>
                <p className='text-xs sm:text-xs font-semibold sm:font-normal text-perce-ink sm:text-perce-ink-secondary whitespace-nowrap'>
                  {formatMoney(displayPrice)}
                </p>
                {cartQtyForProduct > 0 && (
                  <span className='hidden sm:inline text-[10px] text-perce-success font-medium'>
            ...
            <Button
              size='sm'
              variant='outline'
              className='border-perce-ink text-perce-ink hover:bg-perce-surface rounded-md px-2.5 sm:px-4 text-xs font-medium shrink-0 gap-0 sm:gap-1.5'
              onClick={handleAddToCart}
              disabled={!allVariantsSelected}
            ...
            <Button
              size='sm'
              className='bg-perce-cta hover:bg-perce-cta-hover text-perce-ink-inverse rounded-md px-2.5 sm:px-4 text-xs font-semibold shrink-0 whitespace-nowrap'
              onClick={handleBuyNow}
              disabled={!allVariantsSelected}>
              {t("buy_now")}
            </Button>
...
function ProductReviewsSection({ productId }: { productId: string }) {
  ...
  return (
    <div id='product-reviews-section' className='max-w-screen-xl mx-auto px-4 sm:px-6 py-12 border-t border-perce-line'>
      {/* Header */}
      <div className='flex items-center justify-between mb-8'>
        <div>
          <h2 className='text-xl font-medium text-perce-ink'>
        ...
        <Button
          variant='outline'
          size='sm'
          className='rounded-none border-perce-ink text-perce-ink hover:bg-perce-cta-hover hover:text-perce-ink-inverse transition-colors'
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className='mb-10 space-y-4 max-w-lg'>
          <div>
            <label className='block text-sm text-perce-ink-secondary mb-1'>
            ...
            <input
              ...
              maxLength={50}
              className='w-full px-3 py-2 text-sm border border-perce-line outline-none focus:border-perce-ink transition-colors'
                  ...
                  <Star
                    className={`w-6 h-6 ${
                      star <= (hoverRating || formRating)
                        ? "fill-current text-perce-ink"
                        : "text-perce-ink-subtle"
                    } transition-colors`}
                  />
                </button>
              ))}
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={500}
              rows={3}
              className='w-full px-3 py-2 text-sm border border-perce-line outline-none focus:border-perce-ink transition-colors resize-none'
            ...
            {formImagePreview ? (
              <div className='relative w-20 h-20'>
                <img
                  ...
                  alt=''
                  className='w-20 h-20 object-cover border border-perce-line'
                />
                {uploadingImage && (
                  <div className='absolute inset-0 bg-perce-bg/70 flex items-center justify-center'>
                    <span className='text-[10px] text-perce-ink-secondary'>
                ...
                <button
                  ...
                  className='absolute -top-2 -end-2 w-5 h-5 bg-perce-cta text-perce-ink-inverse rounded-full flex items-center justify-center'
              ...
              <input
                ...
                className='text-xs text-perce-ink-secondary'
              />
            )}
          </div>
          <Button
            ...
            className='bg-perce-cta hover:bg-perce-cta-hover text-perce-ink-inverse rounded-none px-8 py-2'>
            {submitting
      ...
      {/* Review list */}
      {reviews.length === 0 ? (
        <p className='text-sm text-perce-ink-muted'>
          {isAr ? "لا توجد تقييمات بعد. كن أول من يقيّم!" : "No reviews yet. Be the first to review!"}
        </p>
      ) : (
        <div className='space-y-6'>
          {reviews.map((review) => (
            <div key={review.id} className='border-b border-perce-line pb-5'>
              <div className='flex items-center justify-between mb-1'>
                <span className='text-sm font-medium text-perce-ink'>{review.userName}</span>
                <span className='text-xs text-perce-ink-muted'>
              ...
              <p className='text-sm text-perce-ink-secondary mt-2 leading-relaxed'>{review.comment}</p>
              {review.imageUrl && (
                <img
                  src={review.imageUrl}
                  alt=''
                  className='mt-3 w-20 h-20 object-cover border border-perce-line'
                />
              )}
            </div>
          ))}
...
function InlineCategoryCarousel({
  ...
  return (
    <div>
      <p className='text-sm font-semibold text-perce-ink mb-3'>{title}</p>
      <div className='relative group/carousel'>
        <button
          onClick={() => scroll("left")}
          disabled={!canScrollLeft}
          className='absolute -start-2 top-1/2 -translate-y-1/2 z-10 hidden md:flex w-7 h-7 items-center justify-center bg-perce-surface-raised border border-perce-line shadow-sm hover:border-perce-ink disabled:opacity-0 transition-all'
        ...
        <button
          ...
          className='absolute -end-2 top-1/2 -translate-y-1/2 z-10 hidden md:flex w-7 h-7 items-center justify-center bg-perce-surface-raised border border-perce-line shadow-sm hover:border-perce-ink disabled:opacity-0 transition-all'
...
function InlineProductCard({
  ...
  return (
    <div
      ...
      className={`flex-none w-[130px] snap-start border transition-colors bg-perce-surface-raised p-2 cursor-pointer ${isSelected ? "border-perce-success ring-1 ring-perce-success" : "border-perce-line hover:border-perce-line-strong"}`}>
      {/* Checkbox */}
      <div
        className={`w-5 h-5 mb-1.5 flex items-center justify-center border-2 transition-all ${isSelected ? "bg-perce-success border-perce-success text-perce-ink-inverse" : "border-perce-line-strong bg-perce-surface-raised"}`}
        aria-hidden='true'>
        {isSelected && <Check className='w-3 h-3' />}
      </div>
      <div className='group/card'>
        <div className='aspect-square bg-perce-surface overflow-hidden mb-2'>
          <img
            src={imageUrl}
            alt={product.name}
        ...
        <h4 className='text-xs font-medium text-perce-ink line-clamp-1'>
          {product.name}
        </h4>
        <div className='flex items-center gap-1.5 mt-0.5'>
          {hasDiscount && (
            <span className='text-[10px] text-perce-ink-muted line-through'>
              {formatMoney(product.price, { locale })}
            </span>
          )}
          <span
            className={`text-xs font-semibold ${hasDiscount ? "text-perce-sale" : "text-perce-ink"}`}>
...
function BottomProductsCarousel({
  ...
  return (
    <div className='bg-perce-surface/50 py-16'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='text-center mb-8'>
          <h2 className='text-2xl font-medium text-perce-ink'>{title}</h2>
          <div className='h-[2px] w-24 bg-perce-cta mt-2 mx-auto' />
        </div>
        ...
        <div className='relative'>
          <button
            onClick={() => scroll("left")}
            disabled={!canScrollLeft}
            className='absolute -left-4 top-1/2 -translate-y-1/2 z-10 hidden md:flex w-10 h-10 items-center justify-center bg-perce-surface-raised border border-perce-line shadow-sm text-perce-ink-secondary hover:text-perce-ink hover:border-perce-ink disabled:opacity-0 transition-all rounded-full'
            aria-label='Previous'>
          ...
          <button
            onClick={() => scroll("right")}
            disabled={!canScrollRight}
            className='absolute -right-4 top-1/2 -translate-y-1/2 z-10 hidden md:flex w-10 h-10 items-center justify-center bg-perce-surface-raised border border-perce-line shadow-sm text-perce-ink-secondary hover:text-perce-ink hover:border-perce-ink disabled:opacity-0 transition-all rounded-full'
            aria-label='Next'>
...
function BottomProductCard({
  ...
  return (
    <div
      ...
      <div className='relative aspect-square bg-perce-surface overflow-hidden'>
        ...
        <div className='absolute bottom-3 start-3 flex gap-2 opacity-0 group-hover/bcard:opacity-100 transition-opacity duration-300'>
          ...
          <Link
            href={productUrl}
            className='w-9 h-9 flex items-center justify-center bg-perce-surface-raised rounded-full shadow-md hover:bg-perce-cta-hover hover:text-perce-ink-inverse transition-colors'
            aria-label='View'>
            <Eye className='w-4 h-4' />
          </Link>
        </div>
        {product.available && !hasDiscount && (
          <span className='absolute top-3 start-3 bg-perce-cta text-perce-ink-inverse text-[10px] font-semibold px-2 py-0.5'>
            {t("new") || "New"}
          </span>
        )}
        {hasDiscount && (
          <span className='absolute top-3 start-3 bg-perce-sale text-perce-ink-inverse text-[10px] font-semibold px-2 py-0.5'>
            {t("sale") || "Sale"}
          </span>
        )}
      </div>
      ...
      <div className='pt-3 text-center'>
        <Link href={productUrl}>
          <h3 className='text-sm font-medium text-perce-ink line-clamp-1 hover:text-perce-ink-secondary transition-colors'>
            {product.name}
          </h3>
        </Link>
        <div className='flex items-center justify-center gap-2 mt-1'>
          {hasDiscount && (
            <span className='text-xs text-perce-ink-subtle line-through'>
              {formatMoney(product.price, { locale })}
            </span>
          )}
          <span
            className={`text-sm font-semibold ${hasDiscount ? "text-perce-sale" : "text-perce-ink"}`}>
            {formatMoney(displayPrice, { locale })}
          </span>
        </div>
        <button
          ...
          data-add-to-cart='true'
          className='mt-3 w-full py-2.5 border border-perce-ink text-perce-ink text-xs font-medium hover:bg-perce-cta-hover hover:text-perce-ink-inverse transition-colors disabled:border-perce-line-strong disabled:text-perce-ink-subtle disabled:cursor-not-allowed disabled:hover:bg-transparent'>
          {product.available ? t("add_to_cart") : t("out_of_stock")}
        </button>
      </div>
    </div>
...
function ExpandableText({ text }: { text: string }) {
  ...
  return (
    <div className='mt-2'>
      <div
        className={`text-sm text-perce-ink-secondary leading-relaxed whitespace-pre-line ${!expanded ? "line-clamp-3" : ""}`}>
        {text}
      </div>
      <button
        onClick={() => setExpanded(!expanded)}
        className='text-sm text-perce-ink underline underline-offset-2 mt-1 hover:text-perce-ink transition-colors'>
        {expanded
...
function MinimalStarRating({
  ...
  for (let i = 0; i < full; i++)
    stars.push(
      <Star
        key={`f${i}`}
        className='h-4 w-4 fill-current text-perce-ink'
      />,
    );
  if (half)
    stars.push(
      <StarHalf key='h' className='h-4 w-4 fill-current text-perce-ink' />,
    );
  for (let i = 0; i < 5 - Math.ceil(rating); i++)
    stars.push(<Star key={`e${i}`} className='h-4 w-4 text-perce-ink-subtle' />);
